    ...
    try:
        ...
        if account_id:
            try:
                ...
                return error_response('INVALID_ACCOUNT_ID', 'account_id must be an integer')
        ...
        current_app.logger.error('Database error listing transactions: %s', e)
...
def get_transaction(transaction_id: str) -> tuple:
    ...
    try:
        ...
        current_app.logger.error('Database error getting transaction %s: %s', transaction_id, e)
...
def create_transaction() -> tuple:
    ...
    try:
        ...
        current_app.logger.info('Created transaction: %s', data['description'])
        ...
        current_app.logger.error('Database error creating transaction: %s', e)
...
def update_transaction(transaction_id: str) -> tuple:
    ...
    try:
            ...
            if data.get('batch_categorize', True):
                ...
                if batch_count > 0:
                    current_app.logger.info(
                        'Batch categorized %s similar transactions to category %s',
                        batch_count,
                        update_doc['category_id'],
                    )

        current_app.logger.info('Updated transaction: %s', transaction_id)
        ...
        current_app.logger.error('Database error updating transaction %s: %s', transaction_id, e)
...
def delete_transaction(transaction_id: str) -> tuple:
    ...
    try:
        ...
        current_app.logger.info('Deleted transaction: %s', transaction_id)

        return success_response(message='Transaction deleted successfully')

    except PyMongoError as e:
        current_app.logger.error('Database error deleting transaction %s: %s', transaction_id, e)
...
def bulk_delete_transactions() -> tuple:
    """
    ...
    try:
        ...
        current_app.logger.info('Bulk deleted %s transactions', result.deleted_count)
        ...
        current_app.logger.error('Database error bulk deleting transactions: %s', e)
